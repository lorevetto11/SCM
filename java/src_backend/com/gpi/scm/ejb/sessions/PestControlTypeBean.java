...
import com.gpi.scm.converters.PestControlTypeConverter;
...
public class PestControlTypeBean extends GenericBean implements PestControlTypeLocal {
	private static final Logger logger = Logger.getLogger(PestControlTypeBean.class);
	...
	public PestControlTypeDto saveType(PestControlTypeDto toSave) throws BusinessException {
		...
		PestControlType type = PestControlTypeConverter.dtoToEntity(toSave);
		type.setOrganization(organization);
		type.setShape(shape);
		PestControlType tmp = entityManager.merge(type);

		return PestControlTypeConverter.entityToDto(tmp, true);
	}

	@Override
	public PestControlTypeDto editType(PestControlTypeDto toSave) throws BusinessException {
		
		PestControlType type = entityManager.find(PestControlType.class, toSave.getId());
	...
	public boolean deleteType(long id) throws BusinessException {
		...
		PestControlType toDelete = entityManager.find(PestControlType.class, id);
	...
	public List<PestControlTypeDto> findTypes(List<Long> organizations) throws BusinessException {
		...
		try {
			...
			List<PestControlType> entities = (List<PestControlType>) entityManager
					.createNamedQuery(PestControlType.NQ_PESTCONTROL_TYPES).setParameter("organizations", organizations)
					.getResultList();
			return PestControlTypeConverter.entityToDto(entities, true);
