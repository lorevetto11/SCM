...
import com.gpi.scm.converters.WaterSupplyTypeConverter;
...
public class WaterSupplyTypeBean extends GenericBean implements WaterSupplyTypeLocal {
	private static final Logger logger = Logger.getLogger(WaterSupplyTypeBean.class);
	...
	public WaterSupplyTypeDto saveType(WaterSupplyTypeDto toSave) throws BusinessException {
		...
		WaterSupplyType type = WaterSupplyTypeConverter.dtoToEntity(toSave);
		type.setOrganization(organization);
		type.setShape(shape);
		WaterSupplyType tmp = entityManager.merge(type);

		return WaterSupplyTypeConverter.entityToDto(tmp, true);
	}

	@Override
	public WaterSupplyTypeDto editType(WaterSupplyTypeDto toSave) throws BusinessException {
		
		WaterSupplyType type = entityManager.find(WaterSupplyType.class, toSave.getId());
	...
	public boolean deleteType(long id) throws BusinessException {
		...
		WaterSupplyType toDelete = entityManager.find(WaterSupplyType.class, id);
	...
	public List<WaterSupplyTypeDto> findTypes(List<Long> organizations) throws BusinessException {
		...
		try {
			...
			List<WaterSupplyType> entities = (List<WaterSupplyType>) entityManager
					.createNamedQuery(WaterSupplyType.NQ_WATERSUPPLY_TYPES).setParameter("organizations", organizations)
					.getResultList();
			return WaterSupplyTypeConverter.entityToDto(entities, true);
