...
import com.gpi.scm.delegates.AirConditioningDelegate;
...
public class AirConditioningService {
	private static final Logger logger = Logger.getLogger(AirConditioningService.class);
	...
	public Response findAll(@HeaderParam("organizationId") Long organizationId) {
		try {
			...
			List<BasePrerequisiteDto> conditionings = AirConditioningDelegate.findAll(organizationId);
			return RestServiceUtils.createHttp200OkResponse(conditionings);
		} catch (Exception be) {
	...
	@GET
	@Path("/airConditioning/{id}")
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response findById(@PathParam("id") long id) {
		try {

			BasePrerequisiteDto conditioning = AirConditioningDelegate.findById(id);

			return RestServiceUtils.createHttp200OkResponse(conditioning);
	...
	@GET
	@Path("/airConditioning/context/{id}")
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response findByContextId(@PathParam("id") long id) {
		try {
			List<Long> organizations = OrganizationDelegate
					.organizationsIdsTree(UserContextHolder.getUser().getOrganization().getId());
			BasePrerequisiteDto supply = AirConditioningDelegate.findByContextId(id, organizations);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response newAirConditioning(BasePrerequisiteDto conditioning) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(conditioning);
			if (!validated.isEmpty()) {
				return ErrorMessage.MyResponse(4, validated);
			}
			return RestServiceUtils.createHttp200OkResponse(AirConditioningDelegate.save(conditioning));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response editAirConditioning(BasePrerequisiteDto conditioning) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(conditioning);
			if (!validated.isEmpty()) {
			...
			return RestServiceUtils.createHttp200OkResponse(AirConditioningDelegate.edit(conditioning));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response deleteAirConditioning(@PathParam("id") long id) {
		try {
			boolean result = AirConditioningDelegate.delete(id);
			if (result) {
