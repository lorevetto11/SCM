...
import com.gpi.scm.delegates.DiagramDelegate;
...
public class DiagramService {
	private static final Logger logger = Logger.getLogger(DiagramService.class);
	...
	public Response findAll(@HeaderParam("organizationId") Long organizationId) {
		try {
			...
			List<DiagramDto> diagrams = DiagramDelegate.findAll(organizationId);
			return RestServiceUtils.createHttp200OkResponse(diagrams);
		} catch (Exception be) {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response newDiagram(DiagramDto diagram) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(diagram);
			if (!validated.isEmpty()) {
				return ErrorMessage.MyResponse(4, validated);
			}

			return RestServiceUtils.createHttp200OkResponse(DiagramDelegate.save(diagram));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response editDiagram(DiagramDto diagram) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(diagram);
			if (!validated.isEmpty()) {
			...
			return RestServiceUtils.createHttp200OkResponse(DiagramDelegate.edit(diagram));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response deleteDiagram(@PathParam("id") long id) {
		try {
			boolean result = DiagramDelegate.delete(id);
			if (result) {
