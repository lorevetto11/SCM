...
import com.gpi.scm.delegates.EquipmentTypeDelegate;
...
public class EquipmentTypeService {
	private static final Logger logger = Logger.getLogger(EquipmentTypeService.class);
	...
	public Response findAllReverseTree(@HeaderParam("organizationId") Long organizationId) {
		try {
			...
			List<EquipmentTypeDto> equipmenTypes = EquipmentTypeDelegate.findEquipmentTypes(organizations);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response newEquipmentTypes(EquipmentTypeDto equipmentType) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(equipmentType);
			if (!validated.isEmpty()) {
				return ErrorMessage.MyResponse(4, validated);
			}
			return RestServiceUtils.createHttp200OkResponse(EquipmentTypeDelegate.saveEquipmentType(equipmentType));
		} catch (Exception be) {
			logger.error("Error creating document sign request. " + be.getMessage());
			String test = ExceptionUtils.getRootCause(be).toString();
			return ErrorMessage.MyResponse(22, test);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response editEquipmentTypes(EquipmentTypeDto equipmentType) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(equipmentType);
			if (!validated.isEmpty()) {
			...
			return RestServiceUtils.createHttp200OkResponse(EquipmentTypeDelegate.editEquipmentType(equipmentType));
	...
	public Response deleteById(@PathParam("id") long id) {
		try {
			...
			boolean result = EquipmentTypeDelegate.deleteById(id);
			if (result) {
