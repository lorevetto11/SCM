...
import com.gpi.scm.delegates.OrganizationCertificationDelegate;
...
public class OrganizationCertificationService {
	private static final Logger logger = Logger.getLogger(OrganizationCertificationService.class);
	...
	public Response findAll(@HeaderParam("organizationId") Long organizationId) {
		try {
			...
			List<OrganizationCertificationDto> orgCertifications = OrganizationCertificationDelegate
					.findOrganizationCertifications(organizations);

			return RestServiceUtils.createHttp200OkResponse(orgCertifications);
		} catch (Exception be) {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response newOrganizationCertifications(OrganizationCertificationDto organization) {
		try {
			...
			return RestServiceUtils.createHttp200OkResponse(
					OrganizationCertificationDelegate.saveOrganizationCertification(organization));
		} catch (Exception be) {
			logger.error("Error creating document sign request. " + be.getMessage());
			String test = ExceptionUtils.getRootCause(be).toString();
			return ErrorMessage.MyResponse(22, test);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response editOrganizationCertifications(OrganizationCertificationDto organization) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(organization);
			if (!validated.isEmpty()) {
			...
			return RestServiceUtils.createHttp200OkResponse(
					OrganizationCertificationDelegate.editOrganizationCertification(organization));
	...
	public Response deleteById(@PathParam("id") long id) {
		try {
			...
			boolean result = OrganizationCertificationDelegate.deleteById(id);
			if (result) {
