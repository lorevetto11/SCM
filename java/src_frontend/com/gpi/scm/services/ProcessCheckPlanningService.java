...
import com.gpi.scm.delegates.ProcessCheckPlanningDelegate;
...
public class ProcessCheckPlanningService {
	private static final Logger logger = Logger.getLogger(ProcessCheckPlanningService.class);
	...
	public Response findAll(@HeaderParam("organizationId") Long organizationId) {
		try {
			...
			List<ProcessCheckPlanningDto> processChecks = ProcessCheckPlanningDelegate
					.findProcessCheckPlannings(organizationId);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response newProcessCheckPlannings(ProcessCheckPlanningDto processCheckPlanning) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(processCheckPlanning);
			if (!validated.isEmpty()) {
				return ErrorMessage.MyResponse(4, validated);
			}
			return RestServiceUtils
					.createHttp200OkResponse(ProcessCheckPlanningDelegate.saveProcessCheckPlanning(processCheckPlanning));
		} catch (Exception be) {
			logger.error("Error creating document sign request. " + be.getMessage());
			String test = ExceptionUtils.getRootCause(be).toString();
			return ErrorMessage.MyResponse(22, test);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response editProcessCheckPlannings(ProcessCheckPlanningDto processCheckPlanning) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(processCheckPlanning);
			if (!validated.isEmpty()) {
			...
			return RestServiceUtils
					.createHttp200OkResponse(ProcessCheckPlanningDelegate.editProcessCheckPlanning(processCheckPlanning));
	...
	public Response deleteById(@PathParam("id") long id) {
		try {
			boolean result = ProcessCheckPlanningDelegate.deleteById(id);
			if (result) {
