...
import com.gpi.scm.delegates.RiskClassDelegate;
...
public class RiskClassService {
	private static final Logger logger = Logger.getLogger(RiskClassService.class);
	...
	public Response findAll(@HeaderParam("organizationId") Long organizationId) {
		try {
			...
			List<RiskClassDto> classes = RiskClassDelegate.findClasses(organizationId);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response newRiskClass(RiskClassDto toSave) {
		try {
			...
			return RestServiceUtils.createHttp200OkResponse(RiskClassDelegate.saveClass(toSave));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response editRiskClass(RiskClassDto toSave) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(toSave);
			if (!validated.isEmpty()) {
			...
			return RestServiceUtils.createHttp200OkResponse(RiskClassDelegate.editClass(toSave));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response deleteRiskClass(@PathParam("id") long id) {
		try {

			boolean result = RiskClassDelegate.delete(id);
			if (result) {
