...
import com.gpi.scm.delegates.SystemCheckPlanningDelegate;
...
public class SystemCheckPlanningService {
	private static final Logger logger = Logger.getLogger(SystemCheckPlanningService.class);
	...
	public Response findAll(@HeaderParam("organizationId") Long organizationId) {
		try {
			...
			List<SystemCheckPlanningDto> systemChecks = SystemCheckPlanningDelegate
					.findSystemCheckPlannings(organizationId);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response newSystemCheckPlannings(SystemCheckPlanningDto sysCheckPlanning) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(sysCheckPlanning);
			if (!validated.isEmpty()) {
				return ErrorMessage.MyResponse(4, validated);
			}
			return RestServiceUtils
					.createHttp200OkResponse(SystemCheckPlanningDelegate.saveSystemCheckPlanning(sysCheckPlanning));
		} catch (Exception be) {
			logger.error("Error creating document sign request. " + be.getMessage());
			String test = ExceptionUtils.getRootCause(be).toString();
			return ErrorMessage.MyResponse(22, test);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response editSystemCheckPlannings(SystemCheckPlanningDto sysCheckPlanning) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(sysCheckPlanning);
			if (!validated.isEmpty()) {
			...
			return RestServiceUtils
					.createHttp200OkResponse(SystemCheckPlanningDelegate.editSystemCheckPlanning(sysCheckPlanning));
	...
	public Response deleteById(@PathParam("id") long id) {
		try {
			boolean result = SystemCheckPlanningDelegate.deleteById(id);
			if (result) {
