...
import com.gpi.scm.delegates.WaterSupplyDelegate;
...
public class WaterSupplyService {
	private static final Logger logger = Logger.getLogger(WaterSupplyService.class);
	...
	public Response findAll(@HeaderParam("organizationId") Long organizationId) {
		try {
			...
			List<BasePrerequisiteDto> supplys = WaterSupplyDelegate.findAll(organizationId);
			return RestServiceUtils.createHttp200OkResponse(supplys);
		} catch (Exception be) {
	...
	public Response findById(@PathParam("id") long id) {
		try {
			List<Long> organizations = OrganizationDelegate
					.organizationsIdsTree(UserContextHolder.getUser().getOrganization().getId());
			BasePrerequisiteDto supply = WaterSupplyDelegate.findById(id, organizations);
	...
	public Response findByContextId(@PathParam("id") long id) {
		try {
			List<Long> organizations = OrganizationDelegate
					.organizationsIdsTree(UserContextHolder.getUser().getOrganization().getId());
			BasePrerequisiteDto supply = WaterSupplyDelegate.findByContextId(id, organizations);
			return RestServiceUtils.createHttp200OkResponse(supply);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response newWaterSupply(BasePrerequisiteDto supply) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(supply);
			if (!validated.isEmpty()) {
				return ErrorMessage.MyResponse(4, validated);
			}

			return RestServiceUtils.createHttp200OkResponse(WaterSupplyDelegate.save(supply));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response editWaterSupply(BasePrerequisiteDto supply) {
		try {
			ValidationUtil valid = new ValidationUtil();
			String validated = valid.isValid(supply);
			if (!validated.isEmpty()) {
			...
			return RestServiceUtils.createHttp200OkResponse(WaterSupplyDelegate.edit(supply));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response deleteWaterSupply(@PathParam("id") long id) {
		try {


			boolean result = WaterSupplyDelegate.delete(id);
			if (result) {
